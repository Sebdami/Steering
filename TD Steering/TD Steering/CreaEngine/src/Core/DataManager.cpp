...
namespace crea
{
	...
	Steering* DataManager::getSteering(string _szName, bool _bCloned)
	{
		...
		if (it == m_pSteerings.end())
		{
			...
			if (!pSteering->loadFromFileJSON(DATAAGENTPATH + _szName))
			{
				delete pSteering;
				cerr << "Unable to open Steering file" << endl;
				return nullptr;
			}
